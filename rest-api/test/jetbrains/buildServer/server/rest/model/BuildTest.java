...
//    vcs.setDAGBased(true); //see jetbrains.buildServer.server.rest.data.ChangeFinderTest.testBranches1() for branches setup  example
  ...
  @Test
  public void testBuildTriggerWithEmptyParameters() {
    final Build build = new Build();
    final BuildType buildType = new BuildType();
    buildType.setId(myBuildType.getExternalId());
    build.setBuildType(buildType);
  ...
  private static final Pattern REDACTED = Pattern.compile("[*]+");
  @Test
  @TestFor(issues = "TW-81244")
  public void testNoSecretParametersForQueuedBuild() {
    final ParameterFactory factory = myFixture.getSingletonService(ParameterFactory.class);
    myBuildType.addParameter(factory.createTypedParameter("password", "pass", "password display='normal'"));
    SQueuedBuild queuedBuild = build().in(myBuildType).addToQueue();
    ...
    assertEquals("Test setup error: we expect exactly one resulting parameter.", 1, model.getResultingProperties().getProperties().size());
    assertEquals("Test setup error: We expect exactly one starting parameter.",1, model.getStartProperties().getProperties().size());


    String resultingParam = model.getResultingProperties().getProperties().get(0).getValue();
    String startingParam = model.getStartProperties().getProperties().get(0).getValue();
    assertTrue("Secret in the resulting param must be redacted.", REDACTED.matcher(resultingParam).matches());
    assertTrue("Secret in starting param must be redacted.", REDACTED.matcher(startingParam).matches());
  }
  ...
  public void testNoParametersForRunningCompositeBuild() {
    ...
    BuildTypeEx bt = BuildServerCreator.createCompositeBuildType(myProject, "composite");
    bt.addParameter(factory.createTypedParameter("password", "pass", "password display='normal'"));

    SRunningBuild runningBuild = build().in(bt).run();
    ...
    assertFalse("Test setup failure: build expected to be running.", runningBuild.isFinished());
  }
