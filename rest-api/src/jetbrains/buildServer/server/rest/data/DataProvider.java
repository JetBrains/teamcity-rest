...
public class DataProvider {
  ...
  public void addAgentToPool(@NotNull final jetbrains.buildServer.serverSide.agentPools.AgentPool agentPool, @NotNull SBuildAgent agent) {
    ...
    try {
      if(agent.isCloudAgent()) {
        // This moves the entire agent type to the pool, not only the agent, TW-40502
        // TODO: Remove this when TW-85526 is fixed
        agentPoolManager.moveAgentTypesToPool(agentPoolId, Collections.singleton(agent.getAgentTypeId()));
      } else {
        agentPoolManager.moveAgentToPool(agentPoolId, (BuildAgentEx) agent);
      }
