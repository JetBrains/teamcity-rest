...
public class DebugRequest {
  ...
  */
  ...
  @GET
  @Path("/promotionReplacement/{originalId}")
  @Produces({"text/plain"})
  public String getPromotionReplacementId(@PathParam("originalId") String originalIdParam) {
    myPermissionChecker.checkGlobalPermission(Permission.MANAGE_SERVER_INSTALLATION);

    long originalId = Long.parseLong(originalIdParam);
    Long replacementId = myBeanContext.getSingletonService(BuildPromotionReplacement.class)
                                      .getReplacementId(originalId);

    if(replacementId == null) {
      throw new NotFoundException("Replacement for promotion with given id was not found.");
    }

    return replacementId.toString();
  }
