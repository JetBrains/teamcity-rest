...
//todo: add vcs roots and others
  ...
  public Project reloadSettingsFile(
    ...
    ProjectsLoader projectsLoader = myBeanContext.getSingletonService(ProjectsLoader.class);

    try {
      SProject project = myProjectFinder.getItem(projectLocator);
      String projectConfigFile = project.getConfigurationFile().getAbsolutePath();
      projectsLoader.reloadProjects(emptyList(), Collections.singleton(new File(projectConfigFile)), emptyList());
    } catch (NotFoundException e) {
      // This server doesn't see requested project. Maybe something happened and we still can recover?
      Locator locator = Locator.locator(projectLocator);
      String projectId = locator.getSingleDimensionValue("id");
      if (projectId == null) {
        throw e;
      }

      File projectsDir = myProjectFinder.getRootProject().getConfigDirectory().getParentFile();
      ...
      projectsLoader.reloadProjects(singleton(new File(projectConfigFile)), emptyList(), emptyList());
    }
  ...
  @GET
  @Path("/{projectLocator}/defaultValueSets")
  @Produces({"application/xml", "application/json"})
  @ApiOperation(value="getDefaultValueSets")
  public TypedValueSets getDefaultValueSets(
    @ApiParam(format = LocatorName.PROJECT) @PathParam("projectLocator") String projectLocator,
    @QueryParam("fields") String fields
  ) {
    SProject project = myProjectFinder.getItem(projectLocator);

    TypedValueSetsProvider provider = myBeanContext.getSingletonService(TypedValueSetsProvider.class);
    return new TypedValueSets(provider.getAll(project), project, new Fields(fields));
  }
