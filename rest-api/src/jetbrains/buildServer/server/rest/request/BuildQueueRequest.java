...
import jetbrains.buildServer.server.rest.data.finder.impl.QueuedBuildFinder;
...
public class  BuildQueueRequest {
  ...
  public ApprovalInfo getApprovalInfo(
    ...
    if (approvableBuildManager.shouldBeApproved(buildPromotionEx).requiresApproval()) {
      ...
      throw new BadRequestException(
         "Trying to access approval status for a queued build that does not have approval configuration"
      );
    }
  }
  ...
  public ApprovalInfo approveQueuedBuild(@ApiParam(format = LocatorName.BUILD) @PathParam("buildLocator") String buildLocator,
    ...
    BuildPromotionEx buildPromotionEx = (BuildPromotionEx)myBuildPromotionFinder.getBuildPromotion(null, buildLocator);
    ApprovalInfo.ApprovalStatus status = ApprovalInfo.ApprovalStatus.resolve(buildPromotionEx, approvableBuildManager);
    if (status != ApprovalInfo.ApprovalStatus.waitingForApproval) {
      throw new BadRequestException("This build cannot be approved, approval status is: " + status);
    }
