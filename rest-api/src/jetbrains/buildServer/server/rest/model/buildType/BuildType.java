...
public class BuildType {
    ...
    return ValueWithDefault.decideDefault(myFields.isIncluded("branches", false, false), // do not include until asked as should only include for branched build types
      () -> {
        ...
        if (locator != null) {
          ...
          return new Branches(result, new SinglePagePagerData(href), nestedFields, myBeanContext);
        }
        href = BuildTypeRequest.getBranchesHref(myBuildType.getBuildType(), null);
        return new Branches(null, new SinglePagePagerData(href), nestedFields, myBeanContext);
      });
  }
  ...
  public Builds getBuilds() {
    ...
    return ValueWithDefault.decideDefault(myFields.isIncluded("builds", false), () -> {
        ...
        if (buildsLocator != null) {
          ...
          return Builds.createFromPrefilteredBuildPromotions(builds, new SinglePagePagerData(buildsHref), buildsFields, myBeanContext);
        }

        String buildsHref = BuildTypeRequest.getBuildsHref(myBuildType.getBuildType());
        return Builds.createPagerOnly(new SinglePagePagerData(buildsHref), buildsFields, myBeanContext);
    });
  }
  ...
  public Investigations getInvestigations() {
    ...
    return ValueWithDefault.decideDefault(myFields.isIncluded("investigations", false, true), () -> {
        ...
        return new Investigations(result, new SinglePagePagerData(InvestigationRequest.getHref(actualLocatorText)), nestedFields, myBeanContext);
    });
  }
