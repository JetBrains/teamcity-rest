...
public class ModelsStaticAnalysisTest {
  ...
  @Test
  public void testPropertiesAreListedInXmlTypeAnnotation() {
    for (Class<?> type : modelTypes) {
      if (type.isAnnotationPresent(XmlType.class)) {
        validateXmlTypeAnnotation(type, getPropertyNamesAnnotated(type).stream().map(ModelProperty::getJavaName).collect(Collectors.toList()));
      }
    }
  }
