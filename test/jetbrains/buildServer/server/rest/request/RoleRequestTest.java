...
public class RoleRequestTest extends BaseServerTestCase {
  ...
  public void test_get_role() {
    ...
    assertTrue(role.permissions.items.stream().anyMatch(
      p -> p.id.equals(Permission.RUN_BUILD.name()) && p.name.equals(Permission.RUN_BUILD.getDescription())
    ));
  ...
  public void test_create_role_with_included_and_permissions() {
    ...
    data.permissions.items.add(new jetbrains.buildServer.server.rest.model.role.Permission(Permission.ADMINISTER_AGENT.name()));
    ...
    assertEquals(Permission.ADMINISTER_AGENT.name(), result.permissions.items.get(0).id);
  ...
  public void test_create_role_invalid_permission() {
    ...
    data.permissions = new Permissions();
    data.permissions.items = new ArrayList<>();
    data.permissions.items.add(new jetbrains.buildServer.server.rest.model.role.Permission("abc"));
    assertExceptionThrown(
  ...
  public void test_add_permission() {
    ...
    assertEquals(Permission.ADMINISTER_AGENT.name(), result2.permissions.items.get(0).id);
  }
