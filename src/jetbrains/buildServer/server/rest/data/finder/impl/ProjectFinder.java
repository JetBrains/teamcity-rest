...
public class ProjectFinder extends AbstractFinder<SProject> {
  ...
  public ItemHolder<SProject> getPrefilteredItems(@NotNull final Locator locator) {
    ...
    if (selectedForUser != null) {
      ...
      return ItemHolder.of(getSelectedProjects(user, getSelectedByUserMode(modeLocator, SelectedByUserMode.SELECTED_AND_UNKNOWN)));
    }
    ...
    try {
      ...
      if (getRootProject().getExternalId().equals(affectedProjectDimension) || getLocator(getRootProject()).equals(affectedProjectDimension)) {
        ...
        return ItemHolder.of(Collections.emptyList());
      }
    ...
    if (name != null) {
      if (parentProject != null) {
        return ItemHolder.of(findProjectsByName(parentProject, name, true));
      }
      String directParent = locator.getSingleDimensionValue(DIMENSION_PROJECT);
      if (directParent != null) {
        return ItemHolder.of(findProjectsByName(getItem(directParent), name, false));
      }
    }

    final String directParent = locator.getSingleDimensionValue(DIMENSION_PROJECT);
    if (directParent != null) {
      return ItemHolder.of(getItem(directParent).getOwnProjects());
    }

    if (parentProject != null) {
      return ItemHolder.of(parentProject.getProjects());
    }

    return ItemHolder.of(myProjectManager.getProjects());
  }
