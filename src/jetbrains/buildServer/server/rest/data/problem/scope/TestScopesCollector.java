...
public class TestScopesCollector {
  ...
  public Stream<TestScope> getItems(@NotNull Locator locator) {
    ...
    if(locator.isAnyPresent(SPLIT_BY_BUILD_TYPE)) {
      ...
      if(split) {
        ...
        scopes = splitByBuildType(scopes, groupParallelTests);
      }
    }
  ...
  @NotNull
  public Stream<TestScope> splitByBuildType(@NotNull Stream<TestScope> testScopes, boolean groupParallelTests) {
    ...
    Function<STestRun, String> getIdAndRememberBuildType = (STestRun testRun) -> {
      ...
      if(groupParallelTests && bt.getProject().isVirtual()) {
        ...
        if(dependees.size() == 1) {
          ...
          // but we won't group parallel tests in such case.
